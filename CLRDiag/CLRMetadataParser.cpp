...
CLRMetadataParser::CLRMetadataParser(PCWSTR path) {
	...
	if (SUCCEEDED(hr)) {
		hr = spDispenser->OpenScope(path, ofRead, IID_IMetaDataImport, reinterpret_cast<IUnknown**>(&_spImport));
		ATLASSERT(SUCCEEDED(hr));
	}
}

CLRMetadataParser::CLRMetadataParser(PVOID p, DWORD size) {
	CComPtr<IMetaDataDispenser> spDispenser;
	auto hr = ::CoCreateInstance(CLSID_CorMetaDataDispenser, nullptr, CLSCTX_ALL,
		IID_IMetaDataDispenser, reinterpret_cast<void**>(&spDispenser));
	if (SUCCEEDED(hr)) {
		hr = spDispenser->OpenScopeOnMemory(p, size, ofRead, IID_IMetaDataImport, reinterpret_cast<IUnknown**>(&_spImport));
		ATLASSERT(SUCCEEDED(hr));
	}
}
